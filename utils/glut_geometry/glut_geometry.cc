...
void glutSolidCone( GLdouble base, GLdouble z_min, GLdouble z_max, GLint slices, GLint stacks )
{
    ...
    double height = z_max;
    double trunc_ratio = (z_max - z_min) / z_max;
    ...
    const double zStep = height * trunc_ratio / ( ( stacks > 0 ) ? stacks : 1 );
    const double rStep = base  * trunc_ratio / ( ( stacks > 0 ) ? stacks : 1 );
    ...
    int n_quad_strips = (z_min == 0.0) ? stacks-1 : stacks;

    for( i=0; i<n_quad_strips; i++ )
    {
        glBegin(GL_QUAD_STRIP);
...
void glutWireCone( GLdouble base, GLdouble z_min, GLdouble z_max, GLint slices, GLint stacks)
{
    ...
    glBegin(GL_LINES);
        ...
        {
            glNormal3d(cost[j]*cosn, sint[j]*cosn, sinn );
            glVertex3d(cost[j]*r,    sint[j]*r,    z_min);
            glVertex3d(z_min,        z_min,        z_max);
        }
